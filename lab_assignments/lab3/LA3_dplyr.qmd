...
**6. One teacher-course combination has some missing values, coded as `NA`. Which instructor has these missing values? Which couse? What variable are the missing values in? Your code output should include *one* row (observation) to answer the question.**
...
For the questions in this section (9 - 11), you don't need to write up any answers -- you only need to show the output of your code (nicely formatted with `kable()`) to respond to each.

::: callout-tip
Helpful function: `slice()`
:::

**9. Which instructor(s) had the lowest average rating for Question 1 ("I learnt a lot during the course.") across all their courses? Include the number of courses the instructor(s) taught in your output**
...
**11. Which instructor(s) with either a doctorate or professor degree had the highest and lowest average percent of students responding to the evaluation across all their courses? Include how many years the professor had worked (seniority) and their sex in your output**
...
3. **creates a new variable called `sen_level` that is “junior” if the instructor has been teaching for 4 years or less (inclusive), “senior” if between 5-8 years (inclusive), and "full" if more than 8 years**
...
::: callout-tip
You may notice that initially your seniority levels and set levels aren't in the same order as my plot! Factor variables are automatially ordered in alphabetical order, but that might not actually make sense for the application. You can re-order the levels of a factor variable using `factor( , levels = ___)`. You might want to look at the documentation or Google examples.
