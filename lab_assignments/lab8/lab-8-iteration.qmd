...
[Download .qmd starter file](lab-8-iteration.qmd)
...
Edit the code below to read in the appropriate datsets that you should have saved
...
# Data from Lab 3
evals <- read_csv("../lab3/teacher_evals.csv") |> 
  rename(sex = gender)
...
**3. Using `map_at()` or `map_if()`, convert the `teacher_id`, `weekday`, `academic_degree`, `time_of_day` `seniority`, and `sex` columns to factors. DO NOT PRINT OUT YOUR NEW DATA FRAME, just show the code.** Hint: You will need to use `bind_cols()` to transform the 
...
This was also a mystery at the time! The code chunk given ran an R script that used `purrr` to iterate over all of the needed datasets and read them in. Let's write a more general function to do this ourselves!
...
**Test it on a directory and show us that it works! Please do not print full datasets, just show us that the output is a list and the names.**
